...
public class IdenticalSubtreeMatcherThetaA implements Matcher {
    ...
    private static class Implementation {
        ...
        private void getHash(ITree node, HashMap<ITree, Integer> quickFind,
                               HashMap<ITree, String> stringMap) {
            for (ITree t: node.postOrder()){
                String tmp = t.getType() + t.getLabel();
                for (ITree child: t.getChildren()){
                    tmp += stringMap.get(child);
                }
                tmp += "\\u2620";

                quickFind.put(t, tmp.hashCode());
                stringMap.put(t, tmp);
            }
...
//            return tmp;
        ...
        @SuppressWarnings( {"checkstyle:AvoidEscapedUnicodeCharacters"})
        @Deprecated
        private String getHash_BACK(ITree node, HashMap<ITree, Integer> quickFind,
                               HashMap<ITree, String> stringMap) {
            String tmp = node.getType() + node.getLabel();
            for (ITree child : node.getChildren()) {
                  tmp += getHash_BACK(child, quickFind, stringMap);
            }
            tmp += "\\u2620";
            quickFind.put(node, tmp.hashCode());
            stringMap.put(node, tmp);
